...
public:
	...
	//作业3.1 定位函数：在单链表中寻找第i个元素节点。若找到，则返回第i个元素节点的地址，否则返回空指针。
	//Node<DataType>* LocateAddress(DataType& data);
	Node<DataType>* LocateAddress(int index);

};
/*
template<class DataType>
Node<DataType>* LinkList<DataType>::LocateAddress(DataType& data)
{
	Node<DataType>* ptemp = head->next;//定义临时变量 从头结点的下一个节点开始查找
	while (ptemp != NULL)// 遍历链表 直到尾节点 结束时为 NULL
	{
		if (ptemp->data == data)// 如果当前节点的数据等于 data
		{
			break;// 找到了，跳出循环 ptmp即为地址
		}
		ptemp = ptemp->next;// 没有 移动到下一个节点
	}
	return ptemp;// 返回找到的节点地址，如果没有找到，返回 NULL
}
*/

template<class DataType>
Node<DataType>* LinkList<DataType>::LocateAddress(int index)
{
